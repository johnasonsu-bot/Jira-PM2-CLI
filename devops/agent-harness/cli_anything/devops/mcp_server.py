"""Read-only stdio MCP adapter. No new database, network listener or shell."""
import argparse
import re
...
INSTRUCTIONS = (
    'Forge DevOps 本地只读工具。统计开发人员工作量时先 list_projects 确认项目，再 rank_workload；'
    ...
    '本服务不能修改任务、重启进程、执行 Shell 或访问文件；不要为了统计创建演示数据。'
)


def create_mcp(url='http://127.0.0.1:8766'):
    backend = Backend(url, actor='Codex/MCP')
    server = FastMCP('Forge DevOps', instructions=INSTRUCTIONS, log_level='WARNING')
...
def main():
    parser = argparse.ArgumentParser(description='Forge DevOps read-only stdio MCP')
    parser.add_argument('--url', default='http://127.0.0.1:8766')
    args = parser.parse_args()
    create_mcp(args.url).run(transport='stdio')
