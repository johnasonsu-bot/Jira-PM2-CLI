...
在安装 Forge 的 Python 环境中，从仓库根目录运行：

```sh
python scripts/verify_object_writes.py
```

此脚本没有指定现有数据库或服务地址的选项，只创建隔离数据库和随机本机端口；测试结束关闭临时服务，保留生成的字段文件、数据库和 `report.json`，路径会输出到终端。不要把这些运行产物提交到 Git。

2026-09-08 已验证：

- Forge Python 全套 109 项、PM2 模拟单元测试 28 项、Node 前端 21 项通过；未运行会操纵真实 PM2 业务进程的生命周期测试。
- 独立脚本通过实际 CLI 验证七类对象的创建、更新、删除、恢复及回读，重复写入不重复应用，过期 revision 被拒绝，版本关联阻止误删，父对象恢复保留子对象的独立删除状态，服务重新打开后结果不变。
- 真实 MCP SDK stdio 测试确认默认 15 个工具、只读模式 8 个工具、写入回读与审计、只读模式拒绝未知写工具。工具协议测试不代表当前 Codex 会话已重新加载目录。
- 在真实导入数据的隔离副本上，迁移后原有 9 张表的数据逐表校验一致；1472 条需求、6689 个场景的原始数据与快照在覆盖层修改及删除/恢复流程后保持不变。测试没有向正式业务库写入演示对象。

源码、测试、本文档和验收脚本可随 Git ZIP 分发；本机业务库、导入源文件及备份不包含在代码 ZIP 中。

### 本机更新验收（2026-09-08）

完成 SQLite 在线备份后，仅重启已核实的 `forge-devops` PM2 进程。更新后的完整性检查为 `ok`；按备份时的原列集合逐表比较，原有 9 张表的行数和内容 SHA-256 全部一致。迁移仅新增 `object_requests`、`object_tombstones` 两张辅助表。

正式服务的 CLI 查询确认 6 个项目、1472 条需求、6689 个场景；新启动的真实 MCP stdio 客户端发现 15 个工具，并通过 `list_objects` 读取 P1 的 543 条需求总数。正式库未执行演示创建、修改、删除或恢复。

本机 Codex 操作 Skill 已同步新命令与安全流程，保留原安装路径说明。当前已连接的 Codex 会话可能仍缓存旧工具目录；需重新连接 MCP 或在重新加载连接的新会话中使用新写工具，不能把上述新客户端验收称为当前会话热更新。
